...
public class AckermannDriver : DriverScript
{
    ...
    public float
        MaxSteeringSpeed = 1,
        SteeringAngleCommand = 0,
        InnerWheelSteeringAngle,
        BreakPedal = 0,
        TireLockRotSpeed = 5,
        WheelFrictionTorque = 1;

    private float thetaAckerman, throttle, tempWheelFriction, angvel, appliedTrq, steeringSpeed = 10, AutoBreak = 0;
    ...
    void FixedUpdate()
    {
        ...
        if (ManualInput)
        {
            float SteeringManualFactor = Mathf.Clamp(5 / ForwardVel, 0, 1);
        ...
        if (BreakOnNullThrottle && throttle == 0)
        {
            AutoBreak += Time.fixedDeltaTime * (1 - AutoBreak);
    ...
    {
        tempWheelFriction = WheelFrictionTorque + Break * MaxBreakingTorque; //breaks on
        Throttle = Mathf.Clamp(Throttle, 0, 1);
        Torque = MaxTorque * Throttle * gear;
        for (int i = 0; i < Wheels.Length; i++)
        {
            angvel = hinges[i].velocity;
            if (Mathf.Abs(angvel) < TireLockRotSpeed && Break > 0.7f) 
                Wheels[i].angularDrag = Break * MaxBreakingTorque; //wheel lock
            else 
                Wheels[i].angularDrag = VelocityDamping;
