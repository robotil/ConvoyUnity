...
public class WaypointDriver : MonoBehaviour
{
    ...
    void init()
    {
        var gos = GameObject.FindGameObjectsWithTag("Waypoint").OrderBy(go => go.name).ToArray(); ;
    ...
    void FixedUpdate()
    {
        ...
        ThrottleManagment();
        TargetMagnet();
       if(drive) driver.Drive(Throttle, Mathf.Clamp(Steer, -maxAng, maxAng));
...
Vector3 dist ;
    private void TargetMagnet()
    {
        ...
        if (drive&&dist.magnitude < TargetMinDistance && WaypointID < TransformList.Count - 1) WaypointID++;
